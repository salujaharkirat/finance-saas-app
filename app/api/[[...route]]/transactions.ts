...
.post(
  ...
  async (ctx) => {
    ...
    const transactionsIdsToDelete = db.$with("transactions_to_delete").as(
      db
    ...
    const data = await db
      .with(transactionsIdsToDelete)
      .delete(transactions)
      .where(
        inArray(transactions.id, sql`(select id from ${transactionsIdsToDelete})`)
      )
